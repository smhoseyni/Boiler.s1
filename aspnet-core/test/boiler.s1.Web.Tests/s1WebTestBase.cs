...
{
    public abstract class s1WebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static s1WebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(s1WebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<s1DbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<s1DbContext, Task> action)
        {
            using (var context = IocManager.Resolve<s1DbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<s1DbContext, Task<T>> func)
        {
            T result;
